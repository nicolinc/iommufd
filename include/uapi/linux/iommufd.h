 ...
 *    This is a per-device option, the object_id must be the iommufd_device ID.
 *    This is only allowed before the device is attached to any IOAS/HWPT. User
 *    can choose to let kernel handle this, by simply ignoring this option or
 *    setting a value 0 to IOMMU_OPTION_SW_MSI_SIZE.
 * @IOMMU_OPTION_SW_MSI_SIZE:
 *    Change the size of the IOMMU mapping region for MSI doorbell(s). This is
 *    a per-device option, the object_id must be the iommufd_device ID. This is
 *    only allowed before the device is attached to any IOAS/HWPT. The value is
 *    in MB, and the minimum value is 1 MB. A value 0 (default) will invalidate
 *    the MSI doorbell base address value set to IOMMU_OPTION_SW_MSI_START.
 */
enum iommufd_option {
