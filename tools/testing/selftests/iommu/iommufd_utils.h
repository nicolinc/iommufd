...
static int _test_cmd_hwpt_alloc_nested(int fd, __u32 idev_id, __u32 parent_id,
				       __u32 *hwpt_id)
{
...
#define test_cmd_hwpt_alloc_nested(idev_id, parent_id, hwpt_id)                \
	ASSERT_EQ(0, _test_cmd_hwpt_alloc_nested(self->fd, idev_id, parent_id, \
						 hwpt_id));
#define test_err_cmd_hwpt_alloc_nested(_errno, idev_id, parent_id, hwpt_id)    \
	EXPECT_ERRNO(_errno,                                                   \
		     _test_cmd_hwpt_alloc_nested(self->fd, idev_id, parent_id, \
						 hwpt_id));
