...
TEST_F(iommufd_ioas, ioas_new_hwpt)
{
	...
	if (self->device_id && self->idev_id) {
		...
		test_cmd_mock_domain_replace(self->device_id, self->ioas_id);
		test_ioctl_destroy(new_hwpt_id);
	} else {
		test_err_ioctl_hwpt_alloc(ENOENT,
					  self->ioas_id, self->idev_id,
					  &new_hwpt_id, !nested);
		test_err_mock_domain_replace(ENOENT,
					     self->device_id, new_hwpt_id);
	}
}
...
TEST_F(iommufd_ioas, ioas_nested_hwpt)
{
	...
	if (self->idev_id) {
		/* Negative tests */
		test_err_ioctl_hwpt_alloc(ENOENT,
					  self->idev_id, self->ioas_id,
					  &test_hwpt_id, !nested);
		test_err_ioctl_hwpt_alloc(EINVAL,
					  self->idev_id, self->idev_id,
					  &test_hwpt_id, !nested);

		/* Allocate two nested hwpts sharing one common parent hwpt */
		test_ioctl_hwpt_alloc(self->ioas_id, self->idev_id,
				      &parent_hwpt_id, !nested);

		test_ioctl_hwpt_alloc(parent_hwpt_id, self->idev_id,
		...
		test_cmd_mock_domain_replace(self->device_id,
					     nested_hwpt_id[0]);
		EXPECT_ERRNO(EBUSY,
		...
		test_cmd_mock_domain_replace(self->device_id, parent_hwpt_id);
		...
		test_err_ioctl_hwpt_alloc(ENOENT,
					  parent_hwpt_id, self->idev_id,
					  &nested_hwpt_id[0], true);
		test_err_ioctl_hwpt_alloc(ENOENT,
					  parent_hwpt_id, self->idev_id,
					  &nested_hwpt_id[1], true);
		test_err_mock_domain_replace(ENOENT, self->device_id,
					     nested_hwpt_id[0]);
		test_err_mock_domain_replace(ENOENT, self->device_id,
					     nested_hwpt_id[1]);
	}
...
TEST_F(iommufd_ioas, hwpt_attach)
{
	...
	if (self->domain_id) {
		test_cmd_mock_domain(self->domain_id, NULL, NULL, NULL);
	} else {
...
TEST_F(iommufd_ioas, device_get_info)
{
	...
	if (self->idev_id) {
		test_cmd_device_get_info(self->idev_id, sizeof(info), &info);
		assert(info.test_reg == IOMMU_DEVICE_INFO_SELFTEST_REGVAL);
	} else {
		test_err_device_get_info(ENOENT, self->idev_id,
					 sizeof(info), &info);
	}
}
...
TEST_F(iommufd_ioas, access_pin)
{
	...
	for (npages = 1; npages < BUFFER_SIZE / PAGE_SIZE; npages++) {
		...
		if (variant->new_hwpt) {
			...
			test_cmd_mock_domain_replace(mock_device_id,
						     mock_hwpt_id);
			test_ioctl_destroy(new_hwpt_id);
		}
...
FIXTURE_SETUP(iommufd_mock_domain)
{
	...
	for (i = 0; i != variant->mock_domains; i++)
		test_cmd_mock_domain(self->ioas_id, &self->device_id[i],
				     &self->domain_ids[i], &self->idev_id[i]);
...
TEST_F(iommufd_mock_domain, replace)
{
	...
	test_cmd_mock_domain_replace(self->device_id[0], ioas_id);
	...
	if (variant->mock_domains == 1)
		test_err_mock_domain_replace(ENOENT, self->device_id[0],
					     self->domain_ids[0]);

	test_cmd_mock_domain_replace(self->device_id[0], ioas_id);
	if (variant->mock_domains >= 2) {
		test_cmd_mock_domain_replace(self->device_id[0],
					     self->domain_ids[1]);
		test_cmd_mock_domain_replace(self->device_id[0],
					     self->domain_ids[1]);
		test_cmd_mock_domain_replace(self->device_id[0],
	...
	test_cmd_mock_domain_replace(self->device_id[0], self->ioas_id);
	test_ioctl_destroy(ioas_id);
}
...
TEST_F(iommufd_mock_domain, alloc_hwpt)
{
	...
	for (i = 0; i != variant->mock_domains; i++) {
		...
		test_cmd_hwpt_alloc(self->idev_id[0], self->ioas_id,
				    &hwpt_id);
