...
TEST_F(iommufd_ioas, ioas_new_hwpt)
{
	...
	if (self->device_id && self->idev_id) {
		test_cmd_hwpt_alloc(self->idev_id, self->ioas_id, &new_hwpt_id);
		test_cmd_mock_domain_replace(self->device_id, new_hwpt_id);
		...
		test_err_cmd_hwpt_alloc(ENOENT, self->idev_id,
...
TEST_F(iommufd_ioas, ioas_nested_hwpt)
{
	...
	if (self->idev_id) {
		/* Negative tests */
		test_err_cmd_hwpt_alloc(ENOENT, self->ioas_id, self->idev_id,
					&test_hwpt_id);
		test_err_cmd_hwpt_alloc(EINVAL, self->idev_id, self->idev_id,
					&test_hwpt_id);
		...
		test_err_cmd_hwpt_alloc(ENOENT, self->idev_id, self->ioas_id,
					&parent_hwpt_id);
		test_err_cmd_hwpt_alloc_nested(ENOENT, self->idev_id,
					       parent_hwpt_id,
					       &nested_hwpt_id[0]);
		test_err_cmd_hwpt_alloc_nested(ENOENT, self->idev_id,
					       parent_hwpt_id,
...
TEST_F(iommufd_ioas, access_pin)
{
	...
	for (npages = 1; npages < BUFFER_SIZE / PAGE_SIZE; npages++) {
		...
		if (variant->new_hwpt) {
			test_cmd_hwpt_alloc(mock_idev_id, self->ioas_id,
					    &new_hwpt_id);
