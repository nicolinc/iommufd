...
TEST_F(iommufd_ioas, ioas_new_hwpt)
{
	...
	if (self->dev_id) {
		...
		test_cmd_mock_domain_replace(self->dev_id, self->ioas_id);
		test_ioctl_destroy(new_hwpt_id);
	} else {
		test_err_ioctl_hwpt_alloc(ENOENT, self->ioas_id, self->dev_id,
					  &new_hwpt_id, !nested);
		test_err_mock_domain_replace(ENOENT, self->dev_id, new_hwpt_id);
	}
}
...
TEST_F(iommufd_ioas, ioas_nested_hwpt)
{
	...
	if (self->dev_id) {
		...
		test_cmd_mock_domain_replace(self->dev_id, nested_hwpt_id[0]);
		EXPECT_ERRNO(EBUSY,
			     _test_ioctl_destroy(self->fd, nested_hwpt_id[0]));

		/* Switch from nested_hwpt_id[0] to nested_hwpt_id[1] */
		test_cmd_mock_domain_replace(self->dev_id, nested_hwpt_id[1]);
		EXPECT_ERRNO(EBUSY,
			     _test_ioctl_destroy(self->fd, nested_hwpt_id[1]));
		test_ioctl_destroy(nested_hwpt_id[0]);

		/* Detach from nested_hwpt_id[1] and destroy it */
		test_cmd_mock_domain_replace(self->dev_id, parent_hwpt_id);
...
FIXTURE_SETUP(iommufd_mock_domain)
{
	...
	for (i = 0; i != variant->mock_domains; i++)
		test_cmd_mock_domain(self->ioas_id, &self->device_id[i], &self->domain_ids[i]);
