...
- IOMMUFD_OBJ_HWPT_NESTED, representing an actual hardware I/O page table
 ...
 - IOMMUFD_OBJ_VIOMMU, representing a group of HW/SW resources of a slice of
   a physical IOMMU instance, which are passed to or shared with a VM. It may
   be some HW-accelerated virtualization features and some SW resources used
   by the VM. For examples yet not confied to the list:
   ...
   * Non-affiliated event reporting
   Such a vIOMMU object generally has the access to nesting parent pagetable to
   support most of HW-accelerated virtualization features. So, a vIOMMU object
   must be created given a nesting parent HWPT_PAGING object, and then it would
   encapsulate the HWPT_PAGING object. Therefore, a vIOMMU object can be used to
   allocate an HWPT_NESTED object in place of the encapsulated HWPT_PAGING.
