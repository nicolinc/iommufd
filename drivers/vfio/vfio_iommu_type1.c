			       ...
			       struct list_head *group_resv_regions)
{
	...
	list_for_each_entry (domain, &iommu->domain_list, next) {
		...
		/* -EINVAL means an incompatible domain, so try next one */
		if (ret == -EINVAL)
			continue;
		if (ret)
			return ERR_PTR(ret);
	...
	enforce_cache_coherency = iommu_group_for_each_dev(group->iommu_group,
				 (void *)IOMMU_CAP_ENFORCE_CACHE_COHERENCY,
				 vfio_iommu_device_capable);
	/*
	 * Try to upgrade the domain we have, it is an iommu driver bug to
	 * report IOMMU_CAP_ENFORCE_CACHE_COHERENCY but fail
	 * enforce_cache_coherency when there are no groups attached to the
	 * domain.
	 */
	if (enforce_cache_coherency && !domain->enforce_cache_coherency) {
		if (domain->domain->ops->enforce_cache_coherency)
			domain->enforce_cache_coherency =
				domain->domain->ops->enforce_cache_coherency(
					domain->domain);
		if (!domain->enforce_cache_coherency) {
			WARN_ON(list_empty(&domain->group_list));
			ret = -EINVAL;
			goto out_free_domain;
		}
	}
