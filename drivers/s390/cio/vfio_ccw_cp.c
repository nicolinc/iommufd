...
struct page_array {
	...
	/* Array that stores the pages need to pin. */
	struct page		**pa_iova_page;
	/* Array that receives the pages pinned. */
	struct page		**pa_page;
...
/*
 * page_array_alloc() - alloc memory for PFNs
 ...
 *
 * Usage of page_array:
 * We expect (pa_nr == 0) and (pa_iova_page == NULL), any field in
 * this structure will be filled in by this function.
 *
 * Returns:
 *         0 if PFNs are allocated
 *   -EINVAL if pa->pa_nr is not initially zero, or pa->pa_iova_page is not NULL
...
static int page_array_alloc(struct page_array *pa, u64 iova, unsigned int len)
{
	...
	pa->pa_iova_page = kcalloc(pa->pa_nr,
				   sizeof(*pa->pa_iova_page) +
				   sizeof(*pa->pa_page),
				   GFP_KERNEL);
	if (unlikely(!pa->pa_iova_page)) {
		pa->pa_nr = 0;
		return -ENOMEM;
	}
	pa->pa_page = pa->pa_iova_page + pa->pa_nr;

	page = pa->pa_iova_page[0] = phys_to_page(pa->pa_iova);
	for (i = 1; i < pa->pa_nr; i++)
		pa->pa_iova_page[i] = ++page;
 ...
 */
static int page_array_pin(struct page_array *pa, struct device *mdev)
{
	int ret = 0;

	ret = vfio_pin_pages(mdev, pa->pa_iova_page, pa->pa_nr,
			     IOMMU_READ | IOMMU_WRITE, pa->pa_page);

	if (ret < 0) {
		goto err_out;
	} else if (ret > 0 && ret != pa->pa_nr) {
		vfio_unpin_pages(mdev, pa->pa_iova_page, ret);
		ret = -EINVAL;
		goto err_out;
	}
...
static void page_array_unpin_free(struct page_array *pa, struct device *mdev)
{
	/* Only unpin if any pages were pinned to begin with */
	if (pa->pa_nr)
		vfio_unpin_pages(mdev, pa->pa_iova_page, pa->pa_nr);
	pa->pa_nr = 0;
	kfree(pa->pa_iova_page);
}
...
static bool page_array_iova_pinned(struct page_array *pa, unsigned long iova)
{
	...
	for (i = 0; i < pa->pa_nr; i++)
		if (pa->pa_iova_page[i] == phys_to_page(iova))
			return true;

	return false;
}
/* Create the list of IDAL words for a page_array. */
static inline void page_array_idal_create_words(
			   ...
			   unsigned long n)
{
	...
	ret = page_array_pin(&pa, mdev);
	if (ret < 0) {
		page_array_unpin_free(&pa, mdev);
		return ret;
	}
				 ...
				 struct channel_program *cp)
{
	...
	if (ccw_is_idal(ccw)) {
		 ...
		 */
		for (i = 0; i < idaw_nr; i++)
			pa->pa_iova_page[i] = phys_to_page(idaws[i]);
	} else {
		/*
...
out_unpin:
	page_array_unpin_free(pa, cp->mdev);
out_free_idaws:
	kfree(idaws);
out_init:
	ccw->cda = 0;
...
void cp_free(struct channel_program *cp)
{
	...
	list_for_each_entry_safe(chain, temp, &cp->ccwchain_list, next) {
		for (i = 0; i < chain->ch_len; i++) {
			page_array_unpin_free(chain->ch_pa + i, cp->mdev);
