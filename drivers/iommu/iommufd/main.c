 ...
 * MMIO region to the iommufd core to receive a vm_pgoff. Then the driver should
 * report to user space this vm_pgoff and the size of the registered region via
 * a prior IOMMU_VIOMMU_ALLOC ioctl in its driver-type data. Use space must use
 * the returned vm_pgoff and size for an mmap() syscal.
 */
static int iommufd_fops_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	/* vm_pgoff carries an index to an mtree entry (immap) */
	immap = mtree_load(&ictx->mt_mmap, vma->vm_pgoff);
	if (!immap)
		return -ENXIO;
	if (size >> PAGE_SHIFT != immap->num_pfns)
		return -ENXIO;
	...
	rc = remap_pfn_range(vma, vma->vm_start, immap->base_pfn, size,
			     vma->vm_page_prot);
