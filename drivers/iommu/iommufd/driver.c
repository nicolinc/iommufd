...
/*
 * Allocate a vm_pgoff to return to the user space to pass in an mmap() syscall
 *
 * Driver should use a per-structure helper in include/linux/iommufd.h
 */
			...
			phys_addr_t base, size_t size, unsigned long *vm_pgoff)
{
	...
	if (WARN_ON_ONCE(!vm_pgoff))
		return -EINVAL;
	if (!PAGE_ALIGNED(base))
		return -EINVAL;
	if (!size || !PAGE_ALIGNED(size))
		return -EINVAL;
	num_pfns = size >> PAGE_SHIFT;
	...
	immap->num_pfns = size >> PAGE_SHIFT;

	rc = mtree_alloc_range(&ictx->mt_mmap, vm_pgoff, immap, immap->num_pfns,
	...
	/* mmap() syscall will right-shift the vm_pgoff to vma->vm_pgoff */
	*vm_pgoff <<= PAGE_SHIFT;
	return 0;
}
...
void _iommufd_destroy_mmap(struct iommufd_ctx *ictx,
			   struct iommufd_object *owner, unsigned long vm_pgoff)
{
	struct iommufd_mmap *immap;

	immap = mtree_erase(&ictx->mt_mmap, vm_pgoff >> PAGE_SHIFT);
