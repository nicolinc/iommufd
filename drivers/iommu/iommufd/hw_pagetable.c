			     ...
			     void *user_data)
{
	...
	hwpt->domain = iommu_domain_alloc_user(dev, parent_domain, user_data);
	if (!hwpt->domain) {
		rc = -ENOMEM;
		goto out_abort;
	}
...
static const u32 iommufd_hwpt_info_size[] = {
	[IOMMU_PGTBL_TYPE_NONE] = 0,
	[IOMMU_PGTBL_TYPE_VTD_S1] = sizeof(struct iommu_hwpt_intel_vtd),
	[IOMMU_PGTBL_TYPE_ARM_SMMUV3_S1] = sizeof(struct iommu_hwpt_arm_smmuv3),
};
...
{
	return ((1 << type) &
			iommuf_supported_pgtbl_types[driver_type]);
}
...
static u32 iommufd_hwpt_invalidate_info_size[] = {
	...
	[IOMMU_PGTBL_TYPE_ARM_SMMUV3_S1] = sizeof(struct iommu_hwpt_invalidate_arm_smmuv3),
};
...
int iommufd_hwpt_invalidate(struct iommufd_ucmd *ucmd)
{
	...
	iommu_iotlb_sync_user(hwpt->domain, cmd);
out_put_hwpt:
