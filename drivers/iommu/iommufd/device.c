				    ...
				    struct iommufd_hwpt_device **out_hdev)
{
	...
	if (out_hdev)
		*out_hdev = hdev;
	hdev->no_domain_attached = attach->no_domain_attach;
	return 0;
				   ...
				   struct iommufd_device_attach_data *attach)
{
	...
	case IOMMUFD_OBJ_HW_PAGETABLE_S1: {
		...
		/* For stage2, no need to attach to real iommu domain as it is
		 * going to be replaced by nesting domain soon. */
		attach->no_domain_attach = true;
