 ...
 * unique within the iommufd_ctx, so we never have to think about complexity
 * when the same groups have been opened in two iommufds.
 */
					       ...
					       struct device *dev)
{
	...
	while (true) {
		cur_igroup = NULL;
		/*
		 * Careful, xa_cmpxchg can unlock while running allowing races
		 * where another thread will install a valid new_group
		 */
					  ...
					  attach_fn do_attach)
{
	...
	hwpt = iommufd_hw_pagetable_alloc(idev->ictx, ioas, NULL, idev,
					  immediate_attach);
	if (IS_ERR(hwpt)) {
		rc = PTR_ERR(hwpt);
		goto out_unlock;
