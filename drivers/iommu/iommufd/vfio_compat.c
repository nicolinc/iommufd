...
{
	struct iommufd_ioas_pagetable *ioaspt = ERR_PTR(-ENODEV);

	xa_lock(&ictx->objects);
	if (!ictx->vfio_ioaspt || !iommufd_lock_obj(&ictx->vfio_ioaspt->obj))
		goto out_unlock;
...
create_compat_ioas(struct iommufd_ctx *ictx)
{
	...
	ioaspt = iommufd_ioas_pagetable_alloc(ictx);
	if (IS_ERR(ioaspt))
		return ioaspt;
	...
	if (!iommufd_lock_obj(&ioaspt->obj))
		WARN_ON(true);
...
int iommufd_vfio_ioas(struct iommufd_ucmd *ucmd)
{
	...
	case IOMMU_VFIO_IOAS_GET:
		ioaspt = get_compat_ioas(ucmd->ictx);
		if (IS_ERR(ioaspt))
			return PTR_ERR(ioaspt);
		cmd->ioas_id = ioaspt->obj.id;
				  ...
				  void __user *arg)
{
	...
	if (unmap.flags & VFIO_DMA_UNMAP_FLAG_ALL)
		...
		rc = iopt_unmap_iova(&ioaspt->iopt, unmap.iova, unmap.size);
	iommufd_put_object(&ioaspt->obj);
	return rc;
}
...
static u64 iommufd_get_pagesizes(struct iommufd_ioas_pagetable *ioaspt)
{
	 ...
	 * compatability. qemu only cares about the first set bit.
	 */
	return ioaspt->iopt.iova_alignment;
}

static int iommufd_fill_cap_iova(struct iommufd_ioas_pagetable *ioaspt,
...
static int iommufd_fill_cap_dma_avail(struct iommufd_ioas_pagetable *ioaspt,
				      struct vfio_info_cap_header __user *cur,
				      size_t avail)
{
...
static int iommufd_vfio_iommu_get_info(struct iommufd_ctx *ictx,
				       void __user *arg)
{
	typedef int (*fill_cap_fn)(struct iommufd_ioas_pagetable * ioaspt,
				   struct vfio_info_cap_header __user * cur,
				   size_t avail);
	...
	info.iova_pgsizes = iommufd_get_pagesizes(ioaspt);
	info.cap_offset = 0;
	...
	for (i = 0; i != ARRAY_SIZE(fill_fns); i++) {
		...
		if (info.argsz > total_cap_size)
			cap_size = fill_fns[i](ioaspt, arg + total_cap_size,
					       info.argsz - total_cap_size);
		else
			cap_size = fill_fns[i](ioaspt, NULL, 0);
		if (cap_size < 0) {
			rc = cap_size;
			goto out_put;
		}
