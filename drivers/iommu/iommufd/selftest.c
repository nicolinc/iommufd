...
static void mock_viommu_destroy(struct iommufd_viommu *viommu)
{
	...
	iommufd_ctx_free_mmap(viommu->ictx, mock_viommu->page_mmap);
		  ...
		  const struct iommu_user_data *user_data)
{
	...
	if (user_data) {
		...
		mock_viommu->page_mmap = iommufd_ctx_alloc_mmap(
			ictx, __pa(mock_viommu->page), PAGE_SIZE, false);
		if (IS_ERR(mock_viommu->page_mmap)) {
			rc = PTR_ERR(mock_viommu->page_mmap);
			mock_viommu->page_mmap = NULL;
			goto err_free_page;
		}
		...
		*mock_viommu->page = data.in_data;
		...
		data.out_mmap_size = PAGE_SIZE;
		data.out_mmap_base = virt_to_phys(mock_viommu->page);
...
err_free_mmap:
	iommufd_ctx_free_mmap(ictx, mock_viommu->page_mmap);
err_free_page:
	free_page((unsigned long)mock_viommu->page);
err_destroy_struct:
