				     ...
				     u32 flags)
{
	...
	sobj->access.ioaspt = get_ioas_pagetable(ucmd, ioas_id);
	if (IS_ERR(sobj->access.ioaspt)) {
		rc = PTR_ERR(sobj->access.ioaspt);
		goto out_free;
	}

	sobj->access.iova = iova;
	sobj->access.length = length;
	rc = iopt_access_pages(&sobj->access.ioaspt->iopt, iova, length, pages,
	...
	iommufd_object_finalize(ucmd->ictx, &sobj->obj);
	iommufd_put_object_keep_user(&sobj->access.ioaspt->obj);
	kvfree(pages);
	return 0;
out_unaccess:
	iopt_unaccess_pages(&sobj->access.ioaspt->iopt, iova, length);
out_put:
	iommufd_put_object(&sobj->access.ioaspt->obj);
out_free:
	kvfree(pages);
out_abort:
	iommufd_object_abort(ucmd->ictx, &sobj->obj);
...
void iommufd_selftest_destroy(struct iommufd_object *obj)
{
	...
	switch (sobj->type) {
	case TYPE_IDEV:
		iopt_table_remove_domain(&sobj->idev.hwpt->ioaspt->iopt,
					 sobj->idev.hwpt->domain);
		iommufd_hw_pagetable_put(sobj->idev.ictx, sobj->idev.hwpt);
		break;
	case TYPE_ACCESS:
		iopt_unaccess_pages(&sobj->access.ioaspt->iopt,
				    sobj->access.iova, sobj->access.length);
		refcount_dec(&sobj->access.ioaspt->obj.users);
		break;
	}
}
