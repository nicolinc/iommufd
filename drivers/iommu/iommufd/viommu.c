...
int iommufd_vdevice_alloc_ioctl(struct iommufd_ucmd *ucmd)
{
	...
	if (viommu->ops && viommu->ops->vdevice_alloc) {
		vdev = viommu->ops->vdevice_alloc(viommu, idev->dev,
						  cmd->virt_id);
		if (IS_ERR(vdev)) {
			rc = PTR_ERR(vdev);
			goto out_unlock_igroup;
	...
	curr = xa_cmpxchg(&viommu->vdevs, cmd->virt_id, NULL, vdev, GFP_KERNEL);
	if (curr) {
