...
static int __iommu_probe_device(struct device *dev, struct list_head *group_list)
{
	...
	if (group->domain) {
		...
		if (ret)
			return ret;
		ret = __iommu_device_set_domain(group, dev, group->domain,
						NULL, 0);
		if (ret)
...
/*
 * Test a future attach for the domain to the device will always fail. This may
 * indicate that the device and domain are incompatible in some way. The actual
 ...
 *
 * Note that domain->ops->test_dev() must keep kernel print free as this op can
 * be called to test compatibility with domains that will simply fail the test,
 * which will result in a kernel log spam.
 */
static int __iommu_domain_test_device(struct iommu_domain *domain,
				      struct device *dev, ioasid_t pasid,
				      struct iommu_domain *old)
