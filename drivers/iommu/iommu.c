			  ...
			  const struct iommu_ops *ops, struct device *hwdev)
{
	...
	iommu->tcb = hwdev && device_tcb_trusted(hwdev);
...
static int iommu_init_device(struct device *dev)
{
	...
	if (dev->iommu->fwspec) {
		...
		/*
		 * Keep the firmware description so a driver rebind after the
		 * device joins the TCB can finish probing it through this
		 * IOMMU.
		 */
		if (iommu && iommu->tcb && !device_tcb_trusted(dev)) {
