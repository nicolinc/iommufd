...
struct group_device {
	...
	char *name;
	/*
	 * Number of pending recoveries that pin this device's HW state at
	 * group->blocking_domain while its group->domain is retained. The
	 * counter increases when:
	 *  - Device (or any alias sibling) is undergoing a reset
	 */
...
/* Propagate block_cnt to every gdev that is DMA aliased to @pdev */
static int __pci_dev_block_dma_alias(struct pci_bus *bus, u16 alias,
				     struct iommu_group *group, int count)
{
	struct group_device *gdev;
	struct pci_dev *pdev;

	pdev = pci_get_domain_bus_and_slot(pci_domain_nr(bus),
	...
	if (!pdev->dev.iommu_group)
		goto out;
	/* Aliased devices must be in the same iommu_group */
	if (WARN_ON(pdev->dev.iommu_group != group))
		goto out;

	gdev = __dev_to_gdev(&pdev->dev);
	if (gdev)
		gdev->block_cnt += count;
out:
	pci_dev_put(pdev);
	return 0;
}

static int pci_dev_block_dma_aliases(struct pci_dev *pdev, u16 alias, void *d)
{
	struct iommu_group *group = d;

	return __pci_dev_block_dma_alias(pdev->bus, alias, group, 1);
}

static int pci_dev_unblock_dma_aliases(struct pci_dev *pdev, u16 alias, void *d)
{
	struct iommu_group *group = d;
...
int pci_dev_reset_iommu_prepare(struct pci_dev *pdev)
{
	...
	if (gdev->block_cnt) {
		ret = pci_for_each_dma_alias(pdev, pci_dev_block_dma_aliases,
					     group);
		goto out;
	}
	...
	/*
	 * Update gdev->block_cnt upon the domain change, as it's used to return
	 * the correct domain in iommu_driver_get_domain_for_dev() that might be
	 * called in a set_dev_pasid callback function.
	 */
	pci_for_each_dma_alias(pdev, pci_dev_block_dma_aliases, group);
...
void pci_dev_reset_iommu_done(struct pci_dev *pdev)
{
	 ...
	 * called in a set_dev_pasid callback function.
	 */
	pci_for_each_dma_alias(pdev, pci_dev_unblock_dma_aliases, group);
