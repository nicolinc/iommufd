...
/* Caller must be an external function that isn't an IOMMU callback */
...
/*
 * Caller must be an IOMMU driver that wants to get the physical domain within
 * an iommu callback function where group->mutex is held
 */
...
int iommu_dev_reset_prepare(struct device *dev)
{
	...
	/* All VFs of a resetting PF should be moved to blocked domain too */
	if (dev_is_pf(dev)) {
		struct pci_dev *vf, *pf = to_pci_dev(dev);
		int i;

		for (i = 0; i < pci_num_vf(pf); i++) {
			vf = pci_get_domain_bus_and_slot(pci_domain_nr(pf->bus),
							 pf->bus->number,
							 pf->devfn + i + 1);
			if (vf) {
				iommu_dev_reset_prepare(&vf->dev);
				pci_dev_put(vf);
			}
		}
	}
	...
	/*
	 * Allow a re-entry. E.g. a PCI VF might be reset individually, while
	 * being attached to ops->blocking_domain already by its PF reset call
	 */
	if (gdev->pending_reset)
		return 0;
	...
	if (dev_is_pci(dev)) {
		for_each_group_device(group, cur) {
			if (cur->dev != dev && dev_is_pci(cur->dev) &&
			    pci_devs_are_dma_aliases(to_pci_dev(cur->dev),
						     to_pci_dev(dev)))
				return 0;
		}
	}
...
void iommu_dev_reset_done(struct device *dev)
{
	...
	/* All VFs of a resetting PF could be restored now */
	if (dev_is_pf(dev)) {
		struct pci_dev *vf, *pf = to_pci_dev(dev);
		int i;
