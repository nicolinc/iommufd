...
static u32 dev_id, test_id = TEST_MAP_SG;
static DEFINE_XARRAY_ALLOC1(device_list);
static unsigned long attrs = DMA_ATTR_SKIP_CPU_SYNC;
...
{
	struct device *dev = xa_load(&device_list, dev_id);
...
static int available_tests_show(struct seq_file *file, void *data)
{
	int i;

	for (i = 0; i < TEST_MAX; i++)
		seq_printf(file, "test_id=%d: %s\n", i, header_str[0][i]);
	return 0;
}
DEFINE_SHOW_ATTRIBUTE(available_tests);

static int available_devs_show(struct seq_file *file, void *data)
{
	struct iommu_domain *domain;
	struct device *dev;
	unsigned long index;

	xa_for_each(&device_list, index, dev) {
		if (!dev)
			continue;
		domain = iommu_get_domain_for_dev(dev);
		if (!domain || domain->type != IOMMU_DOMAIN_DMA)
			continue;
		seq_printf(file, "dev_id=%lu: %s\n", index, dev_name(dev));
	}
	return 0;
}
DEFINE_SHOW_ATTRIBUTE(available_devs);

void iommu_benchmark_init(struct dentry *parent)
{
	struct dentry *benchmark_dir;

	if (!parent)
		return;
	benchmark_dir = debugfs_create_dir("benchmark", parent);
	if (!benchmark_dir)
		return;

	debugfs_create_u32("dev_id", 0600, benchmark_dir, &dev_id);
	...
	debugfs_create_file("start", 0600, benchmark_dir, NULL, &start_fops);
	debugfs_create_file("available_tests", 0600, benchmark_dir, NULL,
			    &available_tests_fops);
	debugfs_create_file("available_devs", 0600, benchmark_dir, NULL,
			    &available_devs_fops);
...
void iommu_benchmark_add_dev(struct device *dev)
{
	u32 id;

	WARN_ON(xa_alloc(&device_list, &id, dev, xa_limit_32b, GFP_KERNEL));
	if (!dev_id)
		dev_id = id;
}
EXPORT_SYMBOL_GPL(iommu_benchmark_add_dev);
