					   ...
					   void *user_data)
{
	...
	struct iommu_hwpt_invalidate_arm_smmuv3 *inv_data = user_data;
	struct arm_smmu_domain *smmu_domain = to_smmu_domain(domain);
	struct arm_smmu_device *smmu = smmu_domain->smmu;
	u64 *cmds;
	int ret;
	...
	for (i = 0; i < inv_data->cmdq_prod; i++) {
		ret = arm_smmu_fix_user_cmd(smmu_domain, &cmds[i * 2]);
		if (ret) {
			inv_data->cmdq_cons = cons_err | i;
			return;
		}
	}
	ret = arm_smmu_cmdq_issue_cmdlist(smmu, cmds, i, true);
	if (ret) {
		inv_data->cmdq_cons = cons_err | i;	// FIXME
		return;
	}
	inv_data->cmdq_cons = inv_data->cmdq_prod;
}
...
static void *arm_smmu_get_mmap_page(struct iommu_domain *domain, size_t pgsize)
{
	...
	if (!smmu_domain->cmdq_user) {
		...
		smmu_domain->cmdq_user_size = pgsize;
	}
