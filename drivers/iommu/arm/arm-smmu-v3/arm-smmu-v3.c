...
static void arm_smmu_setup_ste_by_domain(struct arm_smmu_master *master,
					 u64 *ste)
{
					...
					u64 *ste)
{
	...
	if (master->domain->stage == ARM_SMMU_DOMAIN_NESTED)
		arm_smmu_setup_ste_by_domain(master, ste);
}
...
static void arm_smmu_setup_ste_abort(u64 *ste)
{
	ste[0] |= FIELD_PREP(STRTAB_STE_0_CFG, STRTAB_STE_0_CFG_ABORT);
}

static void arm_smmu_setup_ste_bypass(u64 *ste)
{
	ste[0] |= FIELD_PREP(STRTAB_STE_0_CFG, STRTAB_STE_0_CFG_BYPASS);
	ste[1] |= FIELD_PREP(STRTAB_STE_1_SHCFG, STRTAB_STE_1_SHCFG_INCOMING);
}

static void arm_smmu_sync_strtab_ent(struct arm_smmu_master *master, u32 sid,
				     __le64 *dst, u64 *src)
{
	struct arm_smmu_device *smmu = master->smmu;
	...
	/* See comment in arm_smmu_write_ctx_desc() */
	WRITE_ONCE(dst[0], cpu_to_le64(src[0]));
	arm_smmu_sync_ste_for_sid(smmu, sid);
}

static void arm_smmu_clear_strtab_ent(struct arm_smmu_master *master, u64 *ste)
{
	if (disable_bypass)
...
static void arm_smmu_setup_strtab_ent(struct arm_smmu_master *master, u64 *ste,
				      bool ste_live)
{
...
static void arm_smmu_install_ste_for_dev(struct arm_smmu_master *master)
{
	...
	for (i = 0; i < master->num_streams; ++i) {
		...
		u64 val0 = le64_to_cpu(step[0]);
		bool ste_live = arm_smmu_strtab_ent_is_live(val0);
		 ...
		 */
		if (master->domain)
			arm_smmu_setup_strtab_ent(master, ste, ste_live);
		else
			arm_smmu_clear_strtab_ent(master, ste);
		arm_smmu_sync_strtab_ent(master, sid, step, ste);
