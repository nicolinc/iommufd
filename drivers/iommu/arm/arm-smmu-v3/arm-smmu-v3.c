 ...
 *
 * Return: a newly allocated and sorted array on success, or ERR_PTR
 *
					  ...
					  struct arm_smmu_invs *to_merge)
{
	...
	sort_nonatomic(new_invs->inv, new_invs->num_invs,
		       sizeof(to_merge->inv[0]), arm_smmu_invs_cmp, NULL);
	return new_invs;
}
 ...
 *            the prior arm_smmu_invs_unref() call
 *
 * Return: a newly allocated and sorted array on success, or NULL
 *
 * This function must be locked and serialized with arm_smmu_invs_merge() and
			   ...
			   ioasid_t ssid, struct arm_smmu_domain *smmu_domain)
{
	...
	/*
	 * Set the users of the domain ID entry in the build_invs. This will be
	 * unset in arm_smmu_invs_unref() when the same entry in the per-domain
	 * invs array is being deleted, indicating that this master is the last
	 * user of the domain ID. In this case arm_smmu_invs_flush_iotlb_tags()
	 * must be called to flush all IOTLB entries tagged with the domain ID.
	 */
...
static void arm_smmu_invs_flush_iotlb_tags(struct arm_smmu_invs *invs)
{
	...
	for (i = 0; i != invs->num_invs; i++) {
		...
		/* arm_smmu_invs_unref() unsets users to notify the last user */
