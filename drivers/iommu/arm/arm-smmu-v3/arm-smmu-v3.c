			   ...
			   const __le64 *ent, __le64 *used_bits)
{
	...
	case STRTAB_STE_0_CFG_NESTED:
		used_bits[0] |= STRTAB_STE_0_NESTING_ALLOWED;
		used_bits[1] |= STRTAB_STE_1_NESTING_ALLOWED;
		fallthrough;
				      ...
				      struct device *dev)
{
	 ...
	 */
	state.want_ats = nested_domain->enable_ats &&
			      ...
			      const struct iommu_user_data *user_data)
{
	...
	eats = FIELD_GET(STRTAB_STE_1_EATS, arg.ste[1]);
