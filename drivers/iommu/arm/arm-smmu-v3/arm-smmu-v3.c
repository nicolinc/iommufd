...
static int arm_smmu_probe_device_user(struct device *dev, const void *user_data)
{
	const struct iommu_device_user_arm_smmuv3 *user = user_data;
	struct arm_smmu_master *master = dev_iommu_priv_get(dev);
...
static void arm_smmu_release_device_user(struct device *dev)
{
	struct arm_smmu_master *master = dev_iommu_priv_get(dev);
	struct arm_smmu_stream *stream = &master->streams[0];
	struct arm_smmu_device *smmu = master->smmu;
...
static struct iommu_ops arm_smmu_ops = {
	...
	.probe_device_user	= arm_smmu_probe_device_user,
	.release_device_user	= arm_smmu_release_device_user,
