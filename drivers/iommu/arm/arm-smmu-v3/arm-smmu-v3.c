				      ...
				      __le64 *dst)
{
	...
	if (s1_cfg->set) {
		...
		pr_alert("----%s: %px: setup s1 pgtbl at %llx\n", __func__, smmu_domain->domain, (u64)s1_cfg->cdcfg.cdtab_dma);
	...
	if (s2_cfg->set) {
		...
		pr_alert("----%s: %px: setup s2 pgtbl at %llx\n", __func__, smmu_domain->domain, (u64)vttbr);
			  ...
			  struct iommu_cache_invalidate_info *inv_info)
{
	...
	if (inv_info->cache & IOMMU_CACHE_INV_TYPE_DEV_IOTLB)
		return -ENOENT;
	...
	{
		struct iommu_inv_pasid_info *info = &inv_info->granu.pasid_info;
