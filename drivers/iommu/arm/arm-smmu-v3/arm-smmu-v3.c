				      ...
				      __le64 *dst)
{
	...
	bool abort_both, bypass_both;
	bool s1_live = false, s2_live = false;
	...
	if (!smmu_domain) {
		bypass_both = true;
		abort_both = disable_bypass;
	} else {
		if (!s1_cfg && !s2_cfg)
			bypass_both = true;
		else
			bypass_both = false;
		/* Abort both if stage-1 is set to abort */
		abort_both = smmu_domain->abort;
	}

	if (abort_both || bypass_both) {
		/* Override bypass if abort */
		if (abort_both)
