			       ...
			       unsigned int granule, bool leaf)
{
	 ...
	 * updates in a concurrent attach, to ensure the right order:
	 *  - inv() on new domain sees the updated array with the new ASID
	 *  - HW sees the new IOPTEs if it walks after the STE installation
	 *  - inv() on old domain keeps invalidating with the old ASID until
	 *    seeing the updated array
	 *
	 *  [CPU0]                        | [CPU1]
	 *                                |
	 *  change IOPTEs on new domain:  |
	 ...
	 *    // load the updated array   |   kfree_rcu(old_invs, rcu);
	 *    invs = rcu_dereference();   | }
	 *    ...                         | arm_smmu_install_ste_for_dev {
	 *  }                             |   ...
	 *  ... (context switch)          |   STE = TTB0 // read new IOPTEs
	 *  change IOPTEs on old domain:  |   ...
	 *  arm_smmu_domain_inv_range() { | }
	 *    // load the old invs array  | arm_smmu_install_old_domain_invs {
	 *    invs = rcu_dereference();   |   rcu_assign_pointer(new_invs);
	 *    ...                         |   smp_mb(); // ensure new_invs
...
arm_smmu_install_new_domain_invs(struct arm_smmu_attach_state *state)
{
	 ...
	 * IOPTE updates. See the comments in arm_smmu_domain_inv_range().
	 */
	smp_mb();
