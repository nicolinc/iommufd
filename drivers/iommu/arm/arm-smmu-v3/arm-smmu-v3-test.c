...
static void arm_smmu_v3_invs_test(struct kunit *test)
{
	...
	const int results4[2][3] = { { 1, 2, 3, }, { 1, 1, 1, }, };
	const int results5[2][5] = { { 1, 2, 3, 6, 7, }, { 2, 1, 1, 1, 1, }, };
	const int results6[2][5] = { { 1, 2, 3, 6, 7, }, { 1, 0, 0, 1, 1, }, };
	const int results7[2][5] = { { 1, 2, 3, 6, 7, }, { 0, 0, 0, 0, 0, }, };
	...
	/* Test4: purge test_a (new array) */
	test_b = arm_smmu_invs_purge(test_a, num_dels);
	arm_smmu_v3_invs_test_verify(test, test_b, ARRAY_SIZE(results4[0]),
				     results4[0], results4[1]);
	kfree(test_a);

	/* Test5: merge invs3 (new array) */
	test_a = arm_smmu_invs_merge(test_b, &invs3);
	arm_smmu_v3_invs_test_verify(test, test_a, ARRAY_SIZE(results5[0]),
				     results5[0], results5[1]);
	kfree(test_b);

	/* Test6: unref invs1 (same array) */
	num_dels = arm_smmu_invs_unref(test_a, &invs1);
	arm_smmu_v3_invs_test_verify(test, test_a, ARRAY_SIZE(results6[0]),
				     results6[0], results6[1]);
	KUNIT_EXPECT_EQ(test, num_dels, 2);

	/* Test7: unref invs3 (same array) */
	num_dels = arm_smmu_invs_unref(test_a, &invs3);
	arm_smmu_v3_invs_test_verify(test, test_a, ARRAY_SIZE(results7[0]),
				     results7[0], results7[1]);
	KUNIT_EXPECT_EQ(test, num_dels, 5);

	/* Test8: purge test_a (new array) */
	test_b = arm_smmu_invs_purge(test_a, num_dels);
	KUNIT_EXPECT_EQ(test, test_b->num_invs, 0);
	kfree(test_a);
