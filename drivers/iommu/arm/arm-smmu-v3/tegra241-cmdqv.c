...
struct tegra241_vintf {
	...
	struct iommufd_mmap *page0_mmap;
...
static void tegra241_vintf_user_handle_error(struct tegra241_vintf *vintf)
{
	...
	struct iommu_virq_tegra241_cmdqv virq_data;
	int i;

	for (i = 0; i < LVCMDQ_ERR_MAP_NUM_64; i++)
		virq_data.lvcmdq_err_map[i] =
			readq_relaxed(REG_VINTF(vintf, LVCMDQ_ERR_MAP_64(i)));

	iommufd_viommu_report_irq(viommu, IOMMU_VIRQ_TYPE_TEGRA241_CMDQV,
			    ...
			    const struct iommu_user_data *user_data)
{
	...
	ret = iommu_copy_struct_from_user(
		&arg, user_data, IOMMU_VQUEUE_TYPE_TEGRA241_CMDQV, vcmdq_base);
	if (ret)
		return ERR_PTR(ret);
			   ...
			   const struct iommu_user_data *user_data)
{
	...
	vintf->page0_mmap =
		iommufd_ctx_alloc_mmap(ictx, page0_base, SZ_64K, true);
	if (IS_ERR(vintf->page0_mmap)) {
		ret = PTR_ERR(vintf->page0_mmap);
		vintf->page0_mmap = NULL;
		goto hw_deinit_vintf;
	}

	data.out_vintf_page0_size = SZ_64K;
	data.out_vintf_page0_base = page0_base;
