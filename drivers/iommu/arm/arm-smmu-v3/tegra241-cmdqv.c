 ...
 * @vm_pgoff: Allocated mmappable vm_pgoff for mmap() syscall
...
tegra241_vintf_alloc_lvcmdq_user(struct iommufd_viommu *viommu,
				 unsigned int type, u32 index, u64 base_addr,
				 size_t length)
{
...
static void tegra241_vintf_destroy_lvcmdq_user(struct iommufd_hw_queue *core)
{
...
static void tegra241_cmdqv_destroy_vintf_user(struct iommufd_viommu *viommu)
{
	...
	if (vintf->vm_pgoff)
		iommufd_viommu_destroy_mmap(vintf, vsmmu.core, vintf->vm_pgoff);
				...
				const struct iommu_user_data *user_data)
{
	...
	data.out_vintf_page0_pgoff = vintf->vm_pgoff;
