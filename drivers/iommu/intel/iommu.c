...
{
	switch (user_data>stage) {
	case IOMMU_DOMAIN_USER_STAGE_1:
		return intel_nested_domain_alloc(parent, user_data->stage1_ptr,
						 &user_data->vendor);
	case IOMMU_DOMAIN_USER_STAGE_2:
		return intel_iommu_domain_alloc(IOMMU_DOMAIN_UNMANAGED);
	default:
		return NULL;
	}
}
