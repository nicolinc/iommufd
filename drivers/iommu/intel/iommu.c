...
static int intel_iommu_device_info(struct device *dev, enum iommu_devattr_type type,
				   union iommu_devattr_data *data)
{
	...
	case IOMMU_DEV_INFO_FORCE_SNOOP:
		 ...
		 */
		data->force_snoop = sm_supported(iommu) || ecap_sc_support(iommu->ecap);
		break;
	case IOMMU_DEV_INFO_ADDR_WIDTH:
		data->addr_width = min_t(u32, agaw_to_width(iommu->agaw),
					 cap_mgaw(iommu->cap));
		break;
	default:
		ret = -EINVAL;
