...
interval_tree_span_iter_next_gap(struct interval_tree_span_iter *state)
{
	...
	do {
		...
		state->nodes[1] = interval_tree_iter_next(
			state->nodes[0], state->first_index, state->last_index);
	} while (state->nodes[1] &&
		 (state->nodes[0]->last >= state->nodes[1]->start ||
		  state->nodes[0]->last + 1 == state->nodes[1]->start));
}
